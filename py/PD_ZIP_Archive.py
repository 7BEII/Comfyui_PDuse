...
class PD_ZIP_Packingsave:
    ...
    def sanitize_filename(self, name):
        ...
        # 🌟 终极防御：限制单个文件名长度，防止 Windows Error 123 路径过长崩溃
        if len(name) > 120:
    ...
    def save_to_zip(self, data, filename_prefix, custom_names=None):
            ...
            def process_item(item, base_name):
                ...
                if isinstance(item, (list, tuple)):
                    for sub_item in item:
                        process_item(sub_item, base_name)
                    ...
                    try:
                        ...
                        with open(temp_path, "rb") as f:
                            zipf.writestr(zip_internal_path, f.read())
                        os.remove(temp_path)
                        saved_count += 1
        ...
        return {"ui": {"text": [f"Saved {saved_count} files.\nPath: {zip_path}\nSize: {formatted_size}"]}, 
                "result": ([zip_path], [formatted_size])}
